...
func mainServeHTTP(proto, bindHTTP string, anyHostPorts map[string]bool,
	...
	setupHTTPServer := func(listener net.Listener, nwp string) {
		...
		s := serversCache.registerNewHTTPServer(proto == "https", bindHTTP, nwp, routerInUse)
		...
		if proto == "http" {
			...
			go func(nwp string, listener net.Listener) {
				...
				err := s.server.Serve(limitListener)
				if err != nil {
	...
	if ipv6 != ip_off {
		...
		s := serversCache.getHTTPServer(proto == "https", bindHTTP, "tcp6", routerInUse)
		if s != nil {
			config := s.getTLSConfig()
			config = newTLSConfig(config)
			s.setTLSConfig(config)
	...
	if ipv4 != ip_off {
		// Check if server already exists before opening another
		s := serversCache.getHTTPServer(proto == "https", bindHTTP, "tcp4", routerInUse)
		if s != nil {
			config := s.getTLSConfig()
			config = newTLSConfig(config)
			s.setTLSConfig(config)
...
func getListener(bindAddr, nwp string) (net.Listener, error) {
	if (strings.HasPrefix(bindAddr, "0.0.0.0:") && nwp == "tcp6") ||
		...
		(strings.HasPrefix(bindAddr, "127.0.0.1:") && nwp == "tcp6") ||
