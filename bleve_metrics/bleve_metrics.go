...
// Package bleve_metrics provides a bleve.store.KVStore implementation
// that wraps another, real KVStore implementation, and uses
// go-metrics to track method runtime statistics.
package bleve_metrics
...
func StoreConstructor(config map[string]interface{}) (store.KVStore, error) {
	...
	if !ok || name == "" {
		return nil, fmt.Errorf("bleve_metrics: missing kvStoreName_actual,"+
			" config: %#v", config)
	}

	if name == Name {
		return nil, fmt.Errorf("bleve_metrics: circular kvStoreName_actual")
	}

	ctr := registry.KVStoreConstructorByName(name)
	if ctr == nil {
		return nil, fmt.Errorf("bleve_metrics: no kv store constructor,"+
